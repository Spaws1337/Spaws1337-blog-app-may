...
const BlogDetails = (ctx) => {
    ...
    const handleDelete = async () => {
        try {
            const confirmModal = confirm("Вы уверены что хотите удалить блог?")
    ...
    const handleComment = async () => {
        if(commentText?.length < 2){
            toast.error("Комментарий должен быть больше 2-х символов")
            return
        }
                    ...
                    <div className={classes.comments}>
                        {
                            ...
                            : <h4 className={classes.noComments}>Будь первым кто оставит комментарий!</h4>
                        }
                    </div>
                </div>
            </div>
