...
export async function PUT(req, ctx) {
    ...
    try {
        ...
        if (blog?.authorId?._id.toString() !== decodedToken._id.toString()) {
            return new Response(JSON.stringify({ msg: 'Только автор может редактировать этот блог' }), { status: 403 })
        }
...
export async function DELETE(req, ctx) {
    ...
    try {
        ...
        return new Response(JSON.stringify({msg: 'Блог успешно удален'}), {status: 200})
    } catch (error) {
        return new Response(JSON.stringify(null), { status: 500 }) 
    }
}
