...
export async function DELETE(req, ctx){
    ...
    try {
        ...
        if(comment.authorId._id.toString() !== decodedToken._id.toString()){
            return new Response(JSON.stringify({msg: "Только автор может удалить комментарий"}), {status: 401})
        }

        await Comment.findByIdAndDelete(id)

        return new Response(JSON.stringify({msg: 'Комментарий успешно удален'}), {status: 200})
    } catch (error) {
        return new Response(JSON.stringify(null), {status: 500})
    }
}